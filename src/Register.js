...
class RegisterElement extends React.Component {
    ...
    renderTransactions() {
        ...
        for (const transaction of transactions) {
            ...
            const selected = this.state.selectedTransaction == transaction.id;
...
class Register {
    constructor() {
        ...
        $(document).on('transactionUpdated', (event) => {
            const { transaction, delta } = event.detail;
            ...
            if (this.accountId !== null) {
                ...
                if (this.categoryId === categoryList.unassigned.id) {
                    ...
                    if (!transaction.categories || transaction.categories.length == 0) {
                        ...
                        const i = transaction.categories.findIndex((c) => c.categoryId == this.categoryId);
                        if (i == -1) {
                            this.removeTransaction(transaction.id);
                        }
                        else {
    ...
    removeTransaction(transactionId) {
        const i = this.transactions.findIndex((t) => t.id == transactionId);
        if (i !== -1) {
            const amount = getTransactionAmountForCategory(this.transactions[i], this.categoryId);
    ...
    populateTransactions() {
        const register = React.createElement(
            RegisterElement,
            {
