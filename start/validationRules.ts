...
validator.rule('empty', async (value, [{ table, column}], { pointer, arrayExpressionPointer, errorReporter }) => {
  /**
...
validator.rule('zeroSum', async (values: Array<any>, [{ property }], { pointer, arrayExpressionPointer, errorReporter }) => {
  /**
   * Skip validation when value is not a string. The string
   * schema rule will handle it
   */
